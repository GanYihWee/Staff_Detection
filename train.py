...
def run_training(data_dir, model, model_name, loss_fn, optimizer, epochs = 50, patience = 10):
    ...
    print("Done!")
    print("Weight saved at runs/train/"+opt.name+"/weight/")
    print(device)
    last_model = create_model(resnet, device)
    last_model.load_state_dict(torch.load(model_name))

    torch.save(last_model, "runs/train/"+opt.name+"/"+opt.name+'_'+ opt.model+'.pt')
    print("Model saved at runs/train/"+opt.name+"/")

    eval(last_model, image_datasets['test'], 0.5)
...
if __name__ == '__main__':
    ...
    model_path = "runs/train/"+opt.name+"/weight/"
    ...
    model_name = model_path+opt.name+'_'+ opt.model+'_checkpoint.pt'
    
    run_training('dataset/train_test_val', model, model_name, loss_fn, optimizer, opt.epoch, opt.patience)
